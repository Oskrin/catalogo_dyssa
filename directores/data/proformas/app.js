app.controller('proformasController', function ($scope, $interval) {
	...
	jQuery(function($) {
		                ...
		                if ($("#cantidad").val() == "") {
		                    ...
		                    if ($("#precio_venta").val() == "") {
			                    ...
			                    if (filas.length == 0) {
									...
									// 	url: 'data/proformas/app.php',
									// 	type: 'post',
									// 	data: {cargar_costo_operativo:'cargar_costo_operativo'},
									// 	dataType: 'json',
									...
									// gasto operativo
									desc = 0;
		                            precio = (parseFloat(1.00)).toFixed(3);
			                        ...
			                        jQuery("#table").jqGrid('addRowData', 6833, datarow);
			                        // gasto operativo
						
		                            // ingresar segundo items
		                            desc = $("#descuento").val();
		...
		$scope.methodoshare = function(id) {
		    ...
		    $exampleModal.on("shown.bs.modal", function() {
		        ...
		        $('#link').val('http://administracion.catalogodyssa.com/catalogo/data/reportes/proforma.php?id='+id);
		        $('#link').focus();
		        $('#link').select();
		    });
		}
  ...
  //           });
		...
		$('#btn_3').click(function() {
			if($('#id_proforma').val() == '') {
				$.gritter.add({
					title: 'Seleccione Pedido a Reimprimir',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});	
			} else {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	        ...
	        gridComplete: function() {
				...
				for(var i = 0;i < ids.length;i++) {
					...
					pdf = "<a onclick=\"angular.element(this).scope().methodspdf('"+id_proforma+"')\" title='Reporte Pedido' ><i class='fa fa-file-pdf-o red2' style='cursor:pointer; cursor: hand'> PDF</i></a>"; 
