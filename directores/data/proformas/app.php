<?php        
	...
	if (isset($_POST['buscador_clientes'])) {
		$resultado = $class->consulta("SELECT * FROM clientes WHERE estado = '1' AND id_director = '".$_SESSION['user']['id']."'");
		while ($row = $class->fetch_array($resultado)) {
	...
	// llenar costo operativo
	if (isset($_POST['cargar_costo_operativo'])) {
		$resultado = $class->consulta("SELECT * FROM productos  P, porcentaje_iva V WHERE P.id_porcentaje = V.id AND P.codigo = 'COSOPE' AND P.estado = '1'");
		while ($row = $class->fetch_array($resultado)) {
			$data = array(  'id' => $row[0],
				            'codigo_barras' => $row[1],
				            'codigo' => $row[2],
