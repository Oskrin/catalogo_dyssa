app.controller('directoresController', function ($scope, $route) {
	...
	jQuery(function($) {
	        ...
	        $.ajax({
	            ...
	            success: function(data) {
	                ...
	                if (val == 1) {
	                    ...
	                    $("#identificacion").focus();
	                    $.gritter.add({
							title: 'Error... El Director ya se encuentra Registrado',
		...
		//validacion formulario directores
		$('#form_directores').validate({
			errorElement: 'div',
			...
			messages: {
				...
				porcentaje: {
					required: "Por favor, Indique un Porcentaje",
				},
			},
    	...
    	$("#porcentaje").keypress(ValidNum);
    	// fin

		// guardar formulario
		$('#btn_0').click(function() {
			var respuesta = $('#form_directores').valid();
			
			if (respuesta == true) {
				$('#btn_0').attr('disabled', true);
				var submit = "btn_guardar";
				var formulario = $("#form_directores").serialize();
		...
		jQuery(function($) {
		    ...
		    jQuery(grid_selector).jqGrid({	        
		        datatype: "xml",
		        url: 'data/directores/xml_directores.php',        
		        colNames: ['ID','TIPO DOCUMENTO','IDENTIFICACIÓN','NOMBRES COMPLETOS','TELÉFONO','CELULAR','CIUDAD','DIRECCIÓN','CORREO','ACUMULADO','PORCENTAJE','OBSERVACIONES'],
		        colModel:[      
		            ...
		            {name:'acumulado',index:'acumulado',frozen : true, hidden: false, align:'left',search:false,width: ''},
		            {name:'porcentaje',index:'porcentaje',frozen : true, hidden: false, align:'left',search:false,width: ''},
