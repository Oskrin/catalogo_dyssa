app.controller('factura_ventaController', function ($scope, $interval, loaddatosSRI, $timeout, $route) {
	...
	jQuery(function($) {
		...
		$('#porcentaje').ace_spinner({value:0,min:0,max:100,step:1, on_sides: true, icon_up:'ace-icon fa fa-plus bigger-110', icon_down:'ace-icon fa fa-minus bigger-110', btn_up_class:'btn-success' , btn_down_class:'btn-danger'});
	 ...
	 //        }  
		...
		function datos_empresa() {
			$.ajax({
                ...
                success: function(data) {
                	if (data == null) {
                		swal({
			                title: "Lo sentimos Empresa no Creada",
			                type: "warning",
			            });
			            $("#btn_0").attr("disabled", true);	
                	} else {
                		$("#btn_0").attr("disabled", false);
                		$scope.estab = data.establecimiento;
                		$scope.ptoEmi = data.punto_emision;
                		...
                		$.ajax({
							...
							success: function (data_facturero) {
								if(data_facturero == null) {
									...
									localStorage.setItem("item", data_facturero.num_items);
									$.ajax({
										url: 'data/factura_venta/app.php',
										type: 'post',
										...
										success: function (data_series) {
											if(data_series == null) {
												...
												if (parseInt(data_facturero.finaliza_facturero) >= (parseInt(data_series.serie) + parseInt(1))) {
													...
													localStorage.setItem("secuencial", validado);
												} else {
													swal({
										                title: "Lo sentimos el Número de Factura excedio el Facturero Creado",
										                type: "warning",
										            });
										            $("#btn_0").attr("disabled", true);
												}
			...
			var tipo = 'ruc';
	     	...
	     	$("#ruc").autocomplete({
                ...
                select: function(event, ui) {
		            ...
		            if (ui.item.acumulado == 'NO') {
		            	$("#porcentaje").attr("readOnly", true);
		            } else {
		            	if (ui.item.acumulado == 'SI') {
			                	...
			                	if (parseInt($("#cantidad").val()) > parseInt($("#stock").val())) {
				                    ...
				                    if (filas.length == 0) {
				                        ...
				                        if (repe == 1) {
				                            ...
				                            if(filas.length < parseInt(localStorage.getItem('item'))) {
				                                if ($("#descuento").val() != "") {
				                                    desc = $("#descuento").val();
				                                    precio = (parseFloat($("#precio_venta").val())).toFixed(3);
				                                    multi = (parseFloat($("#cantidad").val()) * parseFloat(precio)).toFixed(3);
				                     ...
				                     for (var t = 0; t < fil.length; t++) {
				                        ...
				                        if (dd['iva'] != 0) {
				                            if(dd['incluye'] == "NO") {
				                                ...
				                                valor_porcentaje = parseFloat((subtotal * porcentaje)/100).toFixed(3);
				                                sub1 = parseFloat(subtotal - valor_porcentaje);
				                                ...
				                                if(dd['incluye'] == "SI") {
				                                    subtotal = parseFloat(dd['total']);
				                                    sub_calculado = (parseFloat((subtotal / ((dd['iva']/100)+1)))).toFixed(3);
				                                    valor_porcentaje = parseFloat((sub_calculado * porcentaje)/100).toFixed(3);
				                                    sub2 = parseFloat(sub_calculado - valor_porcentaje).toFixed(3);
				                    ...
				                    $("#porcentaje_director").val(parseFloat(valor_porcentaje).toFixed(2));
								...
								if($('#select_tipo_comprobante').val() == '') {
									...
									if(filas.length == 0) {
						                ...
						                $.ajax({
						                    type: "POST",
						                    url: "data/factura_venta/app.php",
						                    data: formulario +"&btn_guardar=" + submit + "&campo1=" + string_v1 + "&campo2=" + string_v2 + "&campo3=" + string_v3 + "&campo4=" + string_v4 + "&campo5=" + string_v5 + "&campo6=" + string_v6 + "&id_notas=" + localStorage.getItem('id_notas'),
						                    success: function(data) {
						                    	var id = data;
						                    	...
						                    	if (tipo_comprobante == 1) {
						                    		...
						                    		if (tipo_comprobante == 21) {
						                    			bootbox.alert("Gracias! Por su Información Nota de Entrega Agregada Correctamente!", function() {
										        			var myWindow = window.open('data/reportes/nota_venta.php?hoja=A5&id='+id,'popup','width=900,height=650');
															location.reload();
														});
						                    		}
		...
		$('#btn_cargar').click(function() {
			...
			ids = jQuery("#table5").jqGrid('getGridParam','selarrrow');
			if (ids == '') {
				$.gritter.add({
					title: 'Seleccione Mínimo una Nota de Entrega',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});	
			} else {
				...
				$.ajax({
                    type: "POST",
                    url: "data/factura_venta/app.php",
                    data: "detalle_notas_entrega=" + 'detalle_notas_entrega' + "&ids=" + ids,
                    dataType: 'json',
                    success: function(data) {
                    	if (data['items'] != null) {
                    		if (parseInt(data['items']) > parseInt(localStorage.getItem('item'))) {
                    			$.gritter.add({
									title: 'Error... Alcanzo el limite Máximo de Items el Mínimo a Ingresar es:' + localStorage.getItem('item')  ,
									class_name: 'gritter-error gritter-center',
									time: 1000,
								});
                    		} else {
                    			localStorage.setItem("id_notas", data['ids']);
                    			jQuery('#table').jqGrid('clearGridData');
		                    	var tama = data['datos'].length;
								var descuento = 0;
			                    var desc = 0;
			                    var precio = 0;
			                    var multi = 0;
								...
								for (var i = 0; i < tama; i++) {
									desc = data['datos'][i].descuento;
		                            precio = (parseFloat(data['datos'][i].precio)).toFixed(3);
		                            multi = (parseFloat(data['datos'][i].cantidad) * parseFloat(precio)).toFixed(3);
		                            descuento = ((multi * parseFloat(desc)) / 100);
		                            flotante = parseFloat(descuento);
		                            resultado = (Math.round(flotante * Math.pow(10,2)) / Math.pow(10,2)).toFixed(3);
		                            total = (multi - resultado).toFixed(3);
									...
									var datarow = {
		                                ...
		                                detalle: data['datos'][i].descripcion, 
		                                cantidad: data['datos'][i].cantidad, 
		                                precio_u: precio, 
		                                descuento: desc,
		                                cal_des: resultado, 
		                                total: total,
		                                iva: data['datos'][i].porcentaje,
		                                incluye: data['datos'][i].incluye_iva,
		                                pendiente: data['datos'][i].pendientes
		                            };

		                            jQuery("#table").jqGrid('addRowData',data['datos'][i].id,datarow);
		                            suma_total = suma_total + parseFloat(data['datos'][i].cantidad);
								}
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	        ...
	        autoencode: false,
	        colNames: ['ID','IDENTIFICACIÓN','CLIENTE','SERIE','FECHA EMISIÓN','TOTAL','DIRECTOR@','ACCIÓN'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	            {name:'C.identificacion',index:'C.identificacion', frozen:true, align:'left', search:true, hidden: false},
	            {name:'C.nombres_completos',index:'C.nombres_completos',frozen : true,align:'left', search:true, width: '140px'},
	            ...
	            {name:'D.nombres_completos',index:'D.nombres_completos',frozen : true, search:true, align:'left',width: '140px'},
    ...
    //         	$.ajax({
				...
				// 		$('#porcentaje_director').val(data.porcentaje_director);
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector3).jqGrid({	 
	        ...
	        autoencode: false,
	        colNames: ['ID','IDENTIFICACIÓN','CLIENTE','FECHA EMISIÓN','TOTAL','DIRECTOR@','ACCIÓN'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	            {name:'C.identificacion',index:'C.identificacion', frozen:true, align:'left', search:true, hidden: false},
	            {name:'C.nombres_completos',index:'C.nombres_completos',frozen : true,align:'left', search:true, width: '210px'},
	            {name:'fecha_emision',index:'fecha_emision',frozen : true, align:'left', search:false,width: '120px'},
	            {name:'total_pagar',index:'total_pagar',frozen : true, search:false, align:'left',width: '80px'},
	            {name:'D.nombres_completos',index:'D.nombres_completos',frozen : true, search:true, align:'left',width: '210px'},
	            {name:'accion', index:'accion', editable: false, hidden: false, search:false, frozen: true, editrules: {required: true}, align: 'center', width: '80px'},
	        ],          
	        rowNum: 10,       
	        width:600,
	        ...
	        gridComplete: function() {
				...
				for(var i = 0;i < ids.length;i++) {
					var id_nota = ids[i];
					pdf = "<a onclick=\"angular.element(this).scope().methodspdfnota('"+id_nota+"')\" title='Reporte Nota Entrega' ><i class='fa fa-file-pdf-o red2' style='cursor:pointer; cursor: hand'> PDF</i></a>"; 
	...
	function gridReload() {
		...
		jQuery("#table5").jqGrid('setGridParam',{url:"data/factura_venta/xml_notas_entrega.php?id_cliente="+id_cliente,page:1}).trigger("reloadGrid");
	}
	// fin

	/*jqgrid table 5 buscador*/    
	jQuery(function($) {
	    var grid_selector5 = "#table5";
	    var pager_selector5 = "#pager5";
	    var campo = $('#id_cliente').val();
	    
	    $(window).on('resize.jqGrid', function () {
			$(grid_selector5).jqGrid( 'setGridWidth', $("#myModal .modal-dialog").width()-30);
	    }).trigger('resize');  
	    ...
	    jQuery(grid_selector5).jqGrid({	 
	    	datatype: "xml",      
	        autoencode: false,
	        colNames: ['ID','IDENTIFICACIÓN','CLIENTE','FECHA EMISIÓN','TOTAL','DIRECTOR@'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	            {name:'C.identificacion',index:'C.identificacion', frozen:true, align:'left', search:true, hidden: false},
	            {name:'C.nombres_completos',index:'C.nombres_completos',frozen : true,align:'left', search:true, width: '210px'},
