app.controller('devolucion_ventaController', function ($scope, $interval, loaddatosSRI, $timeout, $route) {
	...
	jQuery(function($) {
	    ...
	    function combo() {
	        $.ajax({
	            type: "POST",
	            url: "data/devolucion_venta/app.php",
	            data: {buscador_series:'buscador_series'},        
	            success: function(resp) {         
	                combo_series = JSON.parse(resp);        
	            }
	        });    
	        return combo_series;
	    }
	    ...
	    function barras() {
            ...
            $.getJSON('data/devolucion_venta/search.php?codigo_barras=' + codigo_barras + '&tipo_precio=' + precio, function(data) {
            	if(data == null) {
					swal({
					...
					if($('#cliente').val() == '') {
						...
						if($('#select_tipo_precio').val() == '') {
							...
							if(filas.length == 0) {
								$('#codigo').focus();
				                $.gritter.add({
									title: 'Ingrese productos a la Devolución',
									class_name: 'gritter-error gritter-center',
									time: 1000,
								});	
				            } else {
				                ...
				                $.ajax({
				                    ...
				                    success: function(data) {
				                    	var id = data;
			                    		bootbox.alert("Gracias! Por su Información Devolución Agregada Correctamente!", function() {
						        			var myWindow = window.open('data/reportes/factura_venta.php?hoja=A5&id='+id,'popup','width=900,height=650');
											location.reload();
										});	
				                    }
