app.controller('fotos_articuloController', function ($scope) {
	...
	$scope.methodimagen = function(id) { 
		$.ajax({
			url: 'data/fotos_articulo/app.php',
			type: 'post',
			data: {llenar_foto:'llenar_foto',id:id},
			dataType: 'json',
			async: true,
			success: function (data) {
				$("#avatar").attr("src","data/fotos_articulo/imagenes/"+data.imagen);
			}
		});
	...
	// activar productos
	$scope.methodactivar = function(id) { 
		bootbox.confirm({
			message: "Está Seguro de Activar este Articulo?",
			buttons: {
			  confirm: {
			...
			callback: function(result) {
				if(result) {
					$.ajax({
						url: 'data/fotos_articulo/app.php',
						type: 'post',
						data: {activar_articulo:'activar_articulo',id:id},
						dataType: 'json',
						async: true,
						success: function (data) {
							var val = data;
	...
	// pasivo articulos
	$scope.methodpasivo = function(id) { 
		bootbox.confirm({
			message: "Está Seguro de Borrar este Articulo?",
			buttons: {
			  confirm: {
				 label: "<i class='ace-icon fa fa-check'></i>Confirmar",
				 className: "btn-sm btn-success",
			...
			callback: function(result) {
				if(result) {
					$.ajax({
						url: 'data/fotos_articulo/app.php',
						type: 'post',
						data: {pasivo_articulo:'pasivo_articulo',id:id},
						dataType: 'json',
						async: true,
						success: function (data) {
							var val = data;
		...
		$.ajax({
			...
			success: function(response) { 
				...
				for (var i = 0; i < response.length; i++) {
					...
					var activo = "<button type='button' class='btn btn-white btn-success btn-sm'  onclick=\"angular.element(this).scope().methodactivar('"+response[i].id+"')\"><span class='fa fa-file-pdf-o green'> Activo</button>";
					var pasivo = "<button type='button' class='btn btn-white btn-pink btn-sm'  onclick=\"angular.element(this).scope().methodpasivo('"+response[i].id+"')\"><span class='fa fa-trash red'> Pasivo</button>";
					var estado = "<span class='label label-success arrowed-in arrowed-in-right'>Activo</span>";
					var estado2 = "<span class='label label-danger arrowed-in arrowed-in-right'>Pasivo</span>";
					var acciones_activo =  vizualizar + ' ' + pasivo;
					var acciones_pasivo =  vizualizar + ' ' + activo;
					...
					if(response[i]['estado'] == '1') {
						tabla.row.add([
					            ...
					            response[i]['precio_costo'],
					            response[i]['precio_minorista'],
					            response[i]['precio_mayorista'],
					            response[i]['stock'],
					            estado,
					            acciones_activo
