...
class xmlapi {
    ...
    public function __construct($host = null, $user = null, $password = null ) {
        ...
        if ($host == null) {
            if ((defined('XMLAPI_HOST')) && (strlen(XMLAPI_HOST) > 0)) {
                ...
                throw new Exception("No host definido");
            }
        } else {
            $this->host = $host;
        }
    ...
    public function set_port($port) {
        ...
        if ($port < 1 || $port > 65535) {
            throw new Exception('non integer or negative integer se pueden pasar a set_port');
        }
    ...
    public function set_protocol($proto) {
        if ($proto != 'https' && $proto != 'http') {
            throw new Exception('https and http son los únicos protocolos que se pueden pasar a set_protocol');
        }
        $this->protocol = $proto;
    }
    ...
    public function set_output($output) {
        if ($output != 'json' && $output != 'xml' && $output != 'array' && $output != 'simplexml') {
            throw new Exception('json, xml, son los únicos valores permitidos para set_output');
        }
        $this->output = $output;
    }
    ...
    public function set_auth_type($auth_type) {
        if ($auth_type != 'hash' && $auth_type != 'pass') {
            throw new Exception('Los dos únicos tipos de autorización permitidos son hash y path');
        }
    ...
    public function set_http_client($client) {
        if (($client != 'curl') && ($client != 'fopen')) {
            throw new Exception('Sólo curl y fopen permite clientes http');
        }
    ...
    public function api1_query($user, $module, $function, $args = array()) {
        if (!isset($module) || !isset($function) || !isset($user)) {
            error_log("api1_query requiere que un módulo y una función sean pasados ​​a él");
